...
abstract class ExportData {
	...
	public function addRow($row, $style = FALSE) {
		$this->write($this->generateRow($row, $style));
	}
	...
	abstract protected function generateRow($row, $style);
...
class ExportDataTSV extends ExportData {
	...
	function generateRow($row, $style) {
...
class ExportDataExcel extends ExportData {
	...
	function generateHeader() {
		...
		$output .= '<Style ss:ID="Default"><Alignment ss:Vertical="Center" ss:Horizontal="Left"/></Style>\n';
		$output .= '<Style ss:ID="Middle"><Alignment ss:Vertical="Center" ss:Horizontal="Center" ss:WrapText="1"/></Style>\n';
		$output .= "</Styles>\n";
	...
	function generateRow($row, $style) {
		$output = '';
		if ($this->set_width) for ($i = 1; $i < 100; $i++) {
			$output .= '<Column ss:Index="'.$i.'" ss:Width="100" />';
		}
		$this->set_width = FALSE;
		$output .= "        <Row>\n";
		foreach ($row as $k => $v) {
			$output .= $this->generateCell($v, $style);
		}
		$output .= "        </Row>\n";
		return $output;
	}

	private function generateCell($item, $style) {
		$output = '';
		...
		if(preg_match("/^-?\d+(?:[.,]\d+)?$/",$item) && (strlen($item) < 12)) {
			$type = 'Number';
		}
